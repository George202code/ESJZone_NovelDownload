...
| 🔁 失败重试 | 下载完成后展示失败章节，支持**多选重试**，仅重下失败项并自动插入 EPUB |
...
| 🔀 分路代理 | 浏览器直连论坛 + 图片独立走代理加速海外 CDN，互不干扰 |
| 🖼 封面优化 | 原生 `<img>` 封面（Sigil / Readest / Calibre 全兼容，不裁切不变形） |
| 📊 日志系统 | 控制台实时进度 + `logs/esjzone.log` 完整调试日志 |
| 🖥 GUI 工具箱 | PyQt6 可视化界面：拖放批量 EPUB 重构、可视化下载（站点选择/断点续传/图片代理/失败重试） |
...
- URL 输入 + 格式校验
- 重跑方式：断点续传 / 清理旧数据
- 无头模式 / 图片代理（可编辑地址 + 提示文字）
- 章节 + 图片独立进度条
- 下载完成后自动汇总失败章节，支持**多选重试**（仅重下失败项并重建 EPUB）
- 并发 / 重试数值配置
- 登录状态显示 + 一键重置
- 实时日志面板
...
```json
{
    ...
    "browser_proxy": null,           // 浏览器代理：null=直连（推荐），有值=走指定代理（与 image_proxy 可不同）
    "browser_bypass_proxy": true,   // （兼容保留）浏览器绕过系统代理，确保论坛可访问
...
删除 `storage_state.json` 重新运行即可重新登录。（GUI 用户可点击下载页的「重置登录」按钮一键完成）
